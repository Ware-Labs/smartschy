...
interface SchematicImageManifest {
  images?: ManifestImageRow[];
}
...
export class SchematicPanel {
  ...
  private pageImageUris: Array<{ pageNumber: number; uri: string }> = [];
...
<html>
  ...
  private resolveRenderedPageUris(): Array<{ pageNumber: number; uri: string }> {
    if (!this.panel) {
      return [];
    }
    const workspaceRoot = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
    ...
    try {
      const payload = JSON.parse(fs.readFileSync(manifestPath, "utf8")) as SchematicImageManifest;
